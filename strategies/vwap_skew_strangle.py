"""vwap_skew_strangle — a four-regime, always-invested short-options strategy.

This is a strategy *module* (importable, spawn-safe, testable) in the BirdsEye
FSM style. Each trading second it classifies the tape into one of four regimes
from two alphas — spot-vs-VWAP deviation and the trailing 15-minute spot range —
and holds the position that regime calls for. It is built so that capital is
almost never idle: regime 4 (a wide strangle) is the fallback we sit in until a
"better" regime (1, 2 or 3) appears.
...
20 lots total = 10 on the CALL branch + 10 on the PUT branch ("10 pairs").
  ...
  * Regimes 3 & 4 take all 10 lots/side in one shot.

The four regimes (evaluated top-down each second; first match wins)
------------------------------------------------------------------
...
class Wait(State):
    ...
    transitions = {
        ...
        "open_fallback": "WIDE",         # regime 4 (always, while room remains)
    }
...
class WideStrangle(State):
    """Regime 4 fallback: short a wide +4/-4 strangle, all 10/side at once. Held
    ...
    def on_enter(self, ctx):
        ctx["episode"]   = "wide"
        ctx["open_legs"] = [(ctx["wide_ce"], "CE"), (ctx["wide_pe"], "PE")]


# --------------------------------------------------------------------------- #
# Strategy                                                                      #
# --------------------------------------------------------------------------- #
class VwapSkewStrangle(StateMachineStrategy):
    states = {
        "WAIT":        Wait(),
        "SKEW_ADD":    SkewAdd(),
        "SKEW_HOLD":   SkewHold(),
    ...
    def __init__(self, broker, lots=10,
        ...
        ctx["credit"]    = 0.0
        ctx["peak_upnl"] = 0.0
        super().__init__("WAIT", broker, name="vwap_skew_strangle", context=ctx)
        ctx["strat"] = self
    ...
    def compute_alphas(self, snap):
        ...
        w_ce,  w_pe  = self._offset(ladder, atm, +4), self._offset(ladder, atm, -4)   # regime 4
        ...
        t_ok,  w_ok  = _ok(t_ce, t_pe),   _ok(w_ce, w_pe)
        ...
        return {
            ...
            "t_ce": t_ce, "t_pe": t_pe, "w_ce": w_ce, "w_pe": w_pe,
            "c1_ok": c1_ok, "c2_ok": c2_ok, "t_ok": t_ok, "w_ok": w_ok,
    ...
    def guard_open_fallback(self, a, c):
        return a["w_ok"] > 0 and self._room_any(a)        # invest only into a quoted wide pair
    ...
    # --- WIDE (regime 4) -> yield to a better regime, or flatten for the close ---
    def guard_wide_to_up(self, a, c):
        return a["dev_bps"] >= self.dev_bps_thresh and a["c1_ok"] > 0 and self._room_skew(a)

    def guard_wide_to_dn(self, a, c):
        return a["dev_bps"] <= -self.dev_bps_thresh and a["c2_ok"] > 0 and self._room_skew(a)
